...
class _Residual_Block_REAL(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, groups=1, wide_width=True, downsample=False, upsample=False):
        ...
        self.conv1 = nn.Conv2d(in_channels, middle_channels, kernel_size=3, stride=1, padding=1, bias=False, groups=groups)
        # if middle_channels < 4:
        #     num_groups = 1
        # else:
        #     num_groups = int(middle_channels / 4)
        # self.GN1 = nn.GroupNorm(num_groups, middle_channels, eps=1e-05, affine=True)
        self.relu1 = nn.LeakyReLU(0.02, inplace=True)
        self.conv2 = nn.Conv2d(middle_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False, groups=groups)
        self.BN2 = nn.BatchNorm2d(out_channels)
        self.SE = QSELayer(out_channels)
...
class QSkipNet(nn.Module):
    def __init__(self, in_chs=16, num_scales=3, delta_chs=16):
        ...
        for i in range(self.num_scales):
            self.QVIT['{}'.format(i)] = QMobViTBlock(opts=None,in_channels=in_chs, attn_unit_dim=in_chs * (self.num_scales - i))
            ...
            self.UP['{}'.format(i)] = nn.ConvTranspose2d(in_channels=self.in_chs * (4 - i), out_channels=self.in_chs * (4 - i), kernel_size=3,
                                                             stride=2, padding=1, output_padding=1)
            in_chs = in_chs + delta_chs

        self.CompressBlock['{}'.format(self.num_scales)] = QDResidual_Block(in_channels=in_chs, out_channels=self.in_chs)
        self.UP['{}'.format(self.num_scales)] = nn.ConvTranspose2d(in_channels=self.in_chs, out_channels=self.in_chs, kernel_size=3,
                                                     stride=2, padding=1, output_padding=1)
...
class QDerainNet(nn.Module):
    ...
    # ["x", "r", "xinit", "outPBnet", "outPUnet","outBnet","outQUnet", "xagg", "outr", "out"]
    def feature_extract(self, x):
        fea = []
        fea.append(x)
        r = self.head(x)
        ...
        x = self.tail(x)
        x = res + x
        out_QUnet, out_PQnet = self.QUnet.feature_extract(x_init)
        fea.append(out_PQnet)
        fea.append(x)
        fea.append(out_QUnet)
        x = out_QUnet + x
        fea.append(x)
